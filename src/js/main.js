...
var getBookmarks = async function () {
    var _getBookmarks = new Promise((resolve) => {
        chrome.bookmarks.getTree((b) => {
            resolve(b.length ? b[0] : []);
        });
    });

    var _getHistory = new Promise((resolve) => {
    ...
    const [bookmarksTree, historyData] = await Promise.all([
        _getBookmarks,
        _getHistory,
    ]);
        var folders = _getFolders(bookmarksTree);

        /* folders.folders.unshift({
            info: {
                title: "Recently Visited"
            },
            links: historyData.slice(0, 10)
        }); */
